...
//comunicaçnao com o sistema auxiliar

//bibliotecas para a transmissão dos dados via RF
#include <RFTransmitter.h>
#define NODE_ID          1
#define OUTPUT_PIN       10
RFTransmitter transmitter(OUTPUT_PIN, NODE_ID);
//variáveis de envio dos dados via RF
int t = 0;
...
int interpitch = 0;//verificação na entrada do pitch para nao pegar a flutuação de valores
long int tempo84 = 0;
float lastvelposimf = 0;
...
<body>
    ...
    <div class="wrapper grid-col-8-2 column-gap ">
        ...
        <div class="wrapper grid-col-4-repeat background-rows border-row">
            ...
            <div class="bloco-col pt-1">       
               <div>
                    <p class="p-col-title">RPM Eixo Lento</p>
                ...
                <div class="mt-105 pt-1">
                    <p class="p-col-title">RPM Eixo Rapido</p>
                    <p class="p-dado-sensor">
            ...
            <div class="bloco-col pt-1 border-none">
              <div>
                    <p class="p-col-title">Tensao Sis. Controle</p>
                    <p class="p-dado-sensor">
    ...
    <div id="modosManuais" class="hide">
        <div class="wrapper grid-col-2-8-4 column-gap mt-1">
            ...
            <div class="wrapper grid-col background-rows border-row">
                <div class="bloco-col border-none">
                    <!-- Titulo secao -->
                    <h2>Ajuste Manual de Angulo</h2>
                    <hr class="hr">
                    <!-- Primeira Parte -->
                    <div>
                        <div id="formNacele" class="mt-105">
                            <p class="p-col-title">Ajuste de YAW</p>
                    ...
                    <div>
                        <div id="formPitch" class="mt-105">
                            <p class="p-col-title">Ajuste de Pitch</p>
...
tensaoSis=0;
while(b <= 20){
tensaoSis = (((analogRead(GPIOtSis)* 3.3) / 4096.0)/ 0.2)+tensaoSis-0.1;
b++;
} 
tSis = tensaoSis/20;
...
tensaoPot=0;
while(b <= 20){
tensaoPot = (((analogRead(GPIOtPot)* 3.3) / 4096.0)/ 0.2)+tensaoPot-0.1;
b++;
} 
tPot = tensaoPot/20;
...
void mediapotpitch(){
  ...
  potpitch=0;
  while(i1<20)  {
  potpitch = potpitch + (map(analogRead(GPIOpotpitch), 190, 3550, 45, 0));
  i1++;
  }
...
//para passar para online uma vez quando em idle
//if(operationMode == 2 & millis() >= 180000 & rpm < 50 & velposimf < cutOff & tSis >= 10.8 & tPot >= 11.0 & tempgerador <= 40.0 & tempsistema <= 60.0 & seg1 == 0 & seg2 == 0 & c1 == 0){
// operationMode == 3; 
// c1 = 1;
//}
...
//if((millis()-tempo16) >= temporet & rpm < 50 & velposimf < cutOff2 & tSis >= 10.8 & tPot >= 11.0 & tempgerador <= 40.0 & tempsistema <= 60.0 & operationMode == 1 & & seg1 == 0 & seg2 == 0){
...
if(rpm > rpmmax1 & millis()-tempo10 > 5000){
...
if(tSis <= 9.5 & operationMode != 5){
  operationMode = 4;
}

//Auto-Safe por baixa tensão no sistema de potência
if(tPot <= 10.0 & operationMode != 5){
  operationMode = 4;
}
...
void ajustepitch(){
  ...
  if(medpotpitch >= pitchReq){
    interpitch = medpotpitch - pitchReq;
  }
  if(pitchReq >= medpotpitch){
    interpitch = pitchReq - medpotpitch;
  }
...
//ajuste no sentido horário
      lastpitch = medpotpitch;
      tempo61 = millis();
  while(medpotpitch > pitchReq & interpitch > 1){


//desativação do sistema em caso de demora no ajuste do pitch
if((millis()-tempo60) >= 80000){
    ledcWrite(1, 0);
    ledcWrite(0, 0);
    seg2=1;
    alarme=1;
...
//rotina de parada do motor em caso de travamento 
//tempo máximo de espera 8seg, anula o loop e impede a reentrada(seg2=1)
    if(medpotpitch == lastpitch & (millis()-tempo61) > 8000){
...
//ajuste de pitch de acordo com a velocidade de vento
...
//anula o loop se passar de 80 seg
 ...
 //rotina de parada do motor em caso de travamento 
//tempo máximo de espera 8seg, anula o loop e impede a reentrada(seg2=1)
    if(medpot == lastnacele & (millis()-tempo61) > 8000){
